        ...
        return (
            <div className="main-content" style={{ padding: '40px 60px', backgroundColor: '#020617', minHeight: '100vh', color: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '1.2rem', color: '#64748b' }}>Getting your pay history ready...</div>
                </div>
            </div>
        );
    }
        ...
        <div className="main-content" style={{ padding: '40px 60px', backgroundColor: '#020617', minHeight: '100vh', color: '#f8fafc' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '48px' }}>
                    <div>
                        ...
                        <h1 style={{ fontSize: '2.5rem', fontWeight: '900', margin: 0, letterSpacing: '-0.02em', background: 'linear-gradient(to right, #fff, #94a3b8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>My Payday Hub</h1>
                        <p style={{ color: '#64748b', fontSize: '1.1rem', marginTop: '8px', fontWeight: '500' }}>Track your hard-earned money and where it comes from</p>
                    </div>
                    <div style={{ display: 'flex', gap: '16px' }}>
                        ...
                        <button onClick={() => setIsModalOpen(true)} style={{
                            padding: '12px 28px', borderRadius: '16px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 10px 25px rgba(16, 185, 129, 0.25)', transition: '0.3s'
                        }}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '32px', marginBottom: '48px' }}>
                    {[
                        ...
                        <div key={i} style={{
                            ...
                            <div style={{ position: 'absolute', top: 0, right: 0, width: '100%', height: '100%', background: stat.gradient, opacity: 0.5 }} />
                            <div style={{ position: 'relative', zIndex: 1 }}>
                                ...
                                <div style={{ fontSize: '2.4rem', fontWeight: '900', color: '#fff', marginBottom: '8px', letterSpacing: '-1px' }}>{stat.value}</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
                    ...
                    {/* Employers List */}
                    <div>
                        <h3 style={{ fontSize: '1.25rem', fontWeight: '900', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px', color: '#fff' }}>
                            <Globe size={20} color="#6366f1" /> My Employers
                        </h3>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                <span style={{ color: '#64748b', fontSize: '0.85rem', fontWeight: '600' }}>{stats.count} Paychecks Received</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ color: '#10b981', fontSize: '1.4rem', fontWeight: '900' }}>₹{stats.total.toLocaleString()}</div>
                                        <div style={{ fontSize: '0.75rem', color: '#475569', fontWeight: '800', textTransform: 'uppercase' }}>Total Earned</div>
                                    </div>
                                </div>
                            )) : (
                                <div style={{ padding: '40px', textAlign: 'center', background: '#0f172a', borderRadius: '24px', border: '1px solid #1e293b', color: '#64748b' }}>
                                    <Coffee size={32} style={{ marginBottom: '16px' }} />
                            ...
                            {salaryItems.length > 0 ? salaryItems.slice(0, 8).map((item) => (
                                <div key={item.id} style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                        <div style={{ color: '#10b981', background: 'rgba(16, 185, 129, 0.1)', padding: '8px', borderRadius: '10px' }}>
                                            <Heart size={16} fill="currentColor" />
                                        </div>
                                        <div>
                                            <div style={{ fontWeight: '800', fontSize: '1rem', color: '#e2e8f0' }}>{item.description}</div>
                                            <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: '600' }}>{new Date(item.date).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</div>
                                        </div>
                                    </div>
                                    <div style={{ color: '#10b981', fontWeight: '950', fontSize: '1.2rem' }}>+₹{item.amount.toLocaleString()}</div>
                                </div>
                            )) : (
                                <p style={{ textAlign: 'center', color: '#64748b', padding: '20px' }}>No payment history found.</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            {isModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    <div style={{ background: '#0f172a', padding: '40px', borderRadius: '32px', border: '1px solid #334155', width: '100%', maxWidth: '480px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                            <h2 style={{ fontSize: '1.8rem', fontWeight: '900', margin: 0, color: '#fff' }}>I Got Paid! 🥳</h2>
                        ...
                        <form onSubmit={handleLogPayday} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <label style={{ fontSize: '0.8rem', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase' }}>Who paid you? (Job Name)</label>
                                <input value={employerName} onChange={e => setEmployerName(e.target.value)} placeholder="e.g. Acme Corp or Freelance" style={{ background: '#020617', border: '1px solid #1e293b', padding: '16px', borderRadius: '16px', color: '#fff', fontSize: '1rem', outline: 'none' }} autoFocus />
                            </div>
                            ...
                            <button type="submit" style={{ marginTop: '12px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: '#fff', padding: '18px', borderRadius: '18px', border: 'none', fontWeight: '900', cursor: 'pointer', fontSize: '1.1rem', boxShadow: '0 10px 25px rgba(16, 185, 129, 0.3)' }}>Save This Payday</button>
                        </form>
                    </div>
                </div>
            )}

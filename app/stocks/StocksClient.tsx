                                    ...
                                    <tr key={stock.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.01)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                                        ...
                                        <td style={{ padding: '16px 24px', textAlign: 'center' }}>
                                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                                                <button onClick={(e) => { e.stopPropagation(); setViewingCharges({ type: 'stock', data: stock }); }} title="Estimated Exit Charges" style={{ background: 'none', border: 'none', color: '#6366f1', cursor: 'pointer', padding: '4px', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.2)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
                                                    <Eye size={16} />
                                                </button>
                                ...
                                <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: '16px' }}>
                                    <div>
                                        ...
                                        {(transaction.brokerage || transaction.taxes) && (
                                            <div style={{ fontSize: '0.7rem', color: '#475569', marginTop: '4px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px' }}>
                                                ...
                                                <button
                                                    onClick={(e) => {
                                                        ...
                                                        setViewingCharges({
                                                            type: 'stock',
                                                            data: {
                                                                symbol: stocks.find(s => s.id === transaction.stockId)?.symbol || 'STOCK',
                                                                quantity: transaction.quantity,
                                                                currentPrice: transaction.price
                                                            }
                                                        });
                                                    }}
                                                    style={{ background: 'none', border: 'none', color: '#6366f1', padding: 0, cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                                                >
                                                    <Eye size={12} />
                                                </button>
                                            </div>
            ...
            }
            {
                viewingCharges && (
                    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1100 }}>
                        <div style={{ background: '#0f172a', padding: '32px', borderRadius: '24px', border: '1px solid #334155', width: '100%', maxWidth: '450px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'rgba(99, 102, 241, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6366f1' }}>
                                        <Eye size={20} />
                                    </div>
                                    <h2 style={{ fontSize: '1.25rem', fontWeight: '900', margin: 0 }}>Charge Breakdown</h2>
                                </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                {viewingCharges.type === 'stock' && (() => {
                                    ...
                                    const charges = calculateStockCharges('SELL', stock.quantity, stock.currentPrice, settings);
                                    return (
                                        <>
                                            <div style={{ padding: '16px', borderRadius: '16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '8px' }}>
                                                <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '4px', fontWeight: '700' }}>Estimating for Selling</div>
                                                <div style={{ fontSize: '1.1rem', fontWeight: '900', color: '#fff' }}>{stock.symbol} • {stock.quantity} shares @ ₹{stock.currentPrice.toFixed(2)}</div>
                                            </div>

                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                {[
                                                    { label: 'Brokerage', value: charges.brokerage, sub: settings.brokerageType === 'flat' ? `₹${settings.brokerageValue} flat` : `${settings.brokerageValue}%` },
                                                    { label: 'STT/CTT', value: charges.stt, sub: `${settings.sttRate}% (Delivery)` },
                                                    { label: 'Transaction Charges', value: charges.transactionCharges, sub: `${settings.transactionChargeRate}% (NSE)` },
                                                    { label: 'SEBI Charges', value: charges.sebiCharges, sub: '₹10/crore' },
                                                    { label: 'Stamp Duty', value: charges.stampDuty, sub: '0.015% (Buy only)' },
                                                    ...
                                                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        <div>
                                                            ...
                                                            <div style={{ fontSize: '0.65rem', color: '#475569' }}>{item.sub}</div>
                                                        </div>
                                                        <div style={{ fontWeight: '800', color: item.value > 0 ? '#fff' : '#475569' }}>₹{item.value.toFixed(2)}</div>
                                                    </div>
                                                ))}
                                            </div>

                                            <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                <div style={{ fontWeight: '900', color: '#fff' }}>Total Charges</div>
                                                <div style={{ fontSize: '1.25rem', fontWeight: '950', color: '#6366f1' }}>₹{charges.total.toFixed(2)}</div>
                                            </div>
                                            <p style={{ fontSize: '0.7rem', color: '#64748b', marginTop: '12px', fontStyle: 'italic' }}>* Estimates based on current market price and your configured tax rates.</p>
                                        </>
                                    );
                                })()}
                            </div>

                            <button onClick={() => setViewingCharges(null)} style={{ width: '100%', background: '#1e293b', color: '#fff', padding: '14px', borderRadius: '16px', border: 'none', fontWeight: '800', cursor: 'pointer', marginTop: '24px' }}>
                                Got it
                            </button>
                        </div>
                    </div>

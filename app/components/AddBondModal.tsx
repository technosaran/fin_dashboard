    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const bondData: Omit<Bond, 'id'> = {
                ...
                quantity: quantity,
                avgPrice: avgPrice,
                currentPrice: avgPrice, // Start with avgPrice
            ...
            // This is a bit tricky: update state in context might be async
            // In a real app, we'd get the ID back from addBond
            await addBond(bondData);

            // Since we don't have the ID easily here without extra logic in context,
            // we'll rely on the context refresh or a small delay.
            // Ideally addBond should return the created bond.
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div className="premium-card shadow-glow fade-in" style={{
                ...
                <div style={{ padding: '32px 32px 16px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <h2 style={{ fontSize: '1.50rem', fontWeight: '950', margin: 0, display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <Landmark size={24} style={{ color: 'var(--accent)' }} /> Add Bond Investment
                        </h2>
                        <p style={{ color: 'var(--text-tertiary)', fontSize: '0.85rem', marginTop: '4px' }}>Search by ISIN or name</p>
                    </div>
                ...
                <div style={{ padding: '0 32px 32px 32px', overflowY: 'auto', flex: 1 }}>
                    ...
                    {/* Search Results */}
                    {searchResults.length > 0 && !selectedBond && (
                            ...
                            {searchResults.map(result => (
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <div style={{ fontWeight: '800', fontSize: '1.05rem', marginBottom: '2px' }}>{result.name}</div>
                                        ...
                                        <div style={{ textAlign: 'right' }}>
                                            <div style={{ color: '#f59e0b', fontWeight: '900', fontSize: '1rem' }}>{result.coupon_rate}%</div>
                                            <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>Coupon</div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    {searchQuery.length > 0 && searchResults.length === 0 && !isSearching && !selectedBond && (

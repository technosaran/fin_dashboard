 ...
 */
export async function fetchGoogleFinancePrice(symbol: string, exchange: string = 'NSE'): Promise<{ price: number, previousClose: number } | null> {
    ...
    try {
        ...
        const prevCloseMatch = html.match(/"?previousClose"?\s*:\s*"?([\d,.]+)"?/i) ||

...
const isOption = (instrument: string): boolean => {
    ...
    return upper.includes(' CE') || upper.includes(' PE') ||
        upper.endsWith('CE') || upper.endsWith('PE') ||
        upper.includes('CALL') || upper.includes('PUT');
};
...
export const calculateFnoCharges = (
    ...
    const entryBrokerage = Math.min(FNO_RATES.BROKERAGE_FLAT, entryTurnover * (FNO_RATES.BROKERAGE_PERCENT / 100));
    const exitBrokerage = Math.min(FNO_RATES.BROKERAGE_FLAT, exitTurnover * (FNO_RATES.BROKERAGE_PERCENT / 100));

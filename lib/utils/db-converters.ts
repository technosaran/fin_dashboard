...
// Database row types
export type AccountRow = { id: number; name: string; bank_name: string; type: string; balance: number; currency: string;[key: string]: unknown };
export type TransactionRow = { id: number; date: string; description: string; category: string; type: string; amount: number; account_id?: number | null;[key: string]: unknown };
export type GoalRow = { id: number; name: string; target_amount: number; current_amount: number; deadline: string; category: string; description?: string | null;[key: string]: unknown };
export type FamilyTransferRow = { id: number; date: string; recipient: string; relationship: string; amount: number; purpose: string; notes?: string | null; account_id?: number | null;[key: string]: unknown };
export type StockRow = { id: number; symbol: string; company_name: string; quantity: number; avg_price: number; current_price: number; sector?: string | null; exchange: string; investment_amount: number; current_value: number; pnl: number; pnl_percentage: number; previous_price?: number | null;[key: string]: unknown };
export type StockTransactionRow = { id: number; stock_id?: number | null; transaction_type: string; quantity: number; price: number; total_amount: number; brokerage?: number | null; taxes?: number | null; transaction_date: string; notes?: string | null; account_id?: number | null;[key: string]: unknown };
export type WatchlistRow = { id: number; symbol: string; company_name: string; target_price?: number | null; current_price?: number | null; notes?: string | null;[key: string]: unknown };
export type MutualFundRow = { id: number; name: string; isin?: string | null; scheme_code?: string | null; category?: string | null; units: number; avg_nav: number; current_nav: number; investment_amount: number; current_value: number; pnl: number; pnl_percentage: number; folio_number?: string | null; previous_nav?: number | null;[key: string]: unknown };
export type MutualFundTransactionRow = { id: number; mutual_fund_id?: number | null; transaction_type: string; units: number; nav: number; total_amount: number; transaction_date: string; notes?: string | null; account_id?: number | null;[key: string]: unknown };
export type FnoTradeRow = { id: number; instrument: string; trade_type: string; product: string; quantity: number; avg_price: number; exit_price?: number | null; entry_date: string; exit_date?: string | null; status: string; pnl: number; notes?: string | null; account_id?: number | null;[key: string]: unknown };
...
export const dbFamilyTransferToFamilyTransfer = (dbTransfer: FamilyTransferRow): FamilyTransfer => ({
...
export const dbStockTransactionToStockTransaction = (dbTransaction: StockTransactionRow): StockTransaction => ({
    id: Number(dbTransaction.id),
...
export const dbMutualFundTransactionToMutualFundTransaction = (dbTx: MutualFundTransactionRow): MutualFundTransaction => ({
    id: Number(dbTx.id),
